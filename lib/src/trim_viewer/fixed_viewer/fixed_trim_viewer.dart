...
class FixedTrimViewer extends StatefulWidget {
  ...
  ///Returns the selected start offset
  final Function(Offset start)? onChangeStartOffset;

  ///Returns the selected end offset
  final Function(Offset end)? onChangeEndOffset;
  ...
  const FixedTrimViewer({
    ...
    this.onChangeEndOffset,
    this.onChangeStartOffset,
    this.startOffset,
    this.endOffset,
  });
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) {
      ...
      setState(() {
        ...
        _startPos = widget.startOffset ?? _startPos;
        _endPos = Offset(widget.endOffset?.dx ?? _endPos.dx, _thumbnailViewerH);
  ...
  void _onDragUpdate(DragUpdateDetails details) {
    ...
    if (_dragType == EditorDragType.left) {
      ...
      if ((_startPos.dx + details.delta.dx >= 0) &&
        ...
        _onStartDragged();
        widget.onChangeStartOffset?.call(_startPos);
      }
      ...
      if ((_startPos.dx + details.delta.dx >= 0) &&
        ...
        widget.onChangeEndOffset?.call(_endPos);
      }
    } else {
      _endCircleSize = widget.editorProperties.circleSizeOnDrag;
